...
class ElNinoData(Dataset):
    def __init__(self, file, var_label='ts', lat_range: tuple = (-90, 90), lon_range: tuple = (-180, 180),
        ...
        # print(f'\n\n{self.ds}\n\n')
        #
        # # create lat lon slices for el nino
        # latitudes = slice(-5, 5)
        # longitudes = slice(-170, -120)
        # # prepares smaller cutout for computing nino34 indices
        # self.ds_nino34 = self.ds.sel(lat=latitudes, lon=longitudes)[var_label]
        # self.ds_nino34 = self.ds.sel(lat=latitudes, lon=longitudes)
        # print(f'\n\n{self.ds_nino34}\n\n')
...
nino34_dataset = ElNinoData("./data/ts_Amon_CESM2_piControl_r1i1p1f1.nc", 'ts', (-5, 5), (-170, -120), 3)
# nino34_dataset = ElNinoData("./data/ts_Amon_CESM2_piControl_r1i1p1f1.nc", 'ts', (-31, 32), (130, -70), 0)
end_time = time.time()
print(f'Time it took to prepare dataset : {end_time - start_time}')
# img, label = nino34_dataset[random.randint(0, len(nino34_dataset))]
# print(img)
# print(img.data)
# print(label)
# sys.exit()
...
# Initializing Network and Optimizer (SGD)
