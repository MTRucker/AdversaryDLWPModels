...
# time_start_year = da['time'].data.min().year
# time_end_year = da['time'].data.max().year
# print(f'\nMin : {time_start_year} and Max : {time_end_year}')
# print(f'\n{type(time_start_year)} and {type(time_end_year)}')
# time_step_size = 30
#
# labels_list = []
#
...
#
#     timeslice_30y = da.sel(time=slice(time_start, time_end))
#     # print(f'\n\n{timeslice_30y}\n\n')
#
#     # Calculate mean of Nino 3.4 Index and save within the new dataset
#     anom = nino34_region - nino34_region.mean(dim='time')
#     nino34 = anom.mean(dim=['lat', 'lon'])
#     # print(nino34)
#     # print(nino34.data)
#     # print(type(nino34.data))
#     for i in nino34.data:
...
class ElNinoData(Dataset):
    def __init__(self, file, var_label='ts', lat_range: tuple = (-90, 90), lon_range: tuple = (-180, 180)):
        ...
        self.nino34_30y_meantime = self._compute_nino34_30y(self.ds)
        self.nino34_iterative_meantime = self._compute_nino34_iterative(self.ds)

        print(f'\n 30y : {self.nino34_30y_meantime}\n\n iterative : {self.nino34_iterative_meantime}')

        fig = plt.figure(figsize=(10, 14))
        plt.plot(self.ds['time'].data, self.nino34_30y_meantime, 'r', alpha=0.8)
        plt.plot(self.ds['time'].data, self.nino34_iterative_meantime, 'b', alpha=0.8)
        plt.show()

        fig = plt.figure(figsize=(10, 14))
        plt.plot(self.ds['time'].sel(time=slice("0001-01-15", "0031-12-15")).data, self.nino34_30y_meantime[0:372], 'r', alpha=0.8)
        plt.plot(self.ds['time'].sel(time=slice("0001-01-15", "0031-12-15")).data, self.nino34_iterative_meantime[0:372], 'b', alpha=0.8)
        plt.show()

        sys.exit()
        self.nino_label_list =  1
    ...
    def _compute_nino34_iterative(self, darray):

        # list of all labeled nino34 years
        labels_list = []
        ...
        print(f'\niterative meantime :{len(labels_list)}')
        return labels_list

    def _compute_nino34_30y(self, darray):
